...
% --- Title ---
\title{Model Compression and Pruning for Efficient Deep Learning}

% --- Author Information ---
% Use \IEEEauthorblockN and \IEEEauthorblockA for affiliation
% Use \and to separate authors
\author{
\IEEEauthorblockN{Geetika Khanna}
\IEEEauthorblockA{University of Maryland}
\and
\IEEEauthorblockN{Archit Harsh}
\IEEEauthorblockA{University of Maryland}
\and
\IEEEauthorblockN{Himal Sharma}
\IEEEauthorblockA{University of Maryland}
}
...
\begin{abstract}
    ...
    \begin{enumerate}
        \item Iterative Greedy Magnitude Pruning with Fine-tuning (iGreedy), stopping based on an accuracy threshold, and 
        \item L1 Regularization during training followed by weight thresholding and fine-tuning. 
    \end{enumerate}
    ...
    \href{https://github.com/geetikak13/MLPruning}{\color{blue}MLPruning}.
\end{abstract}

% --- Main Content ---
\IEEEpeerreviewmaketitle % For peer review papers, shows keywords/abstract on title page

\section{Introduction}
% Use \IEEEPARstart for the first paragraph's drop cap
\IEEEPARstart{T}{he} success of Deep Neural Networks (DNNs) across various fields is undeniable. However, this success often comes at the cost of enormous model size and computational complexity. Models like VGG, ResNet, and large language models can have millions or billions of parameters, leading to significant challenges:
\begin{itemize}
...
This formulation combines the task performance (loss) and model complexity (number of parameters) into a single objective function using a trade-off parameter $\lambda$.
...
\begin{itemize}
    ...
    \begin{enumerate} % Outer list for Algorithm
        ...
        \item \textbf{Fine-tuning:} Fine-tune the thresholded network without the L1 penalty.
    \end{enumerate} % End outer list
...
We first trained the base \texttt{SimpleCNN} model using \texttt{scripts/train.py} for 50 epochs, achieving a baseline accuracy of 85.49\%.

\textbf{iGreedy Threshold Pruning:} We ran \texttt{scripts/prune\_igreedy.py} with \texttt{--accuracy\_threshold 84.0} and \texttt{--prune\_step\_amount 0.2}. The script iteratively pruned 10\% of remaining weights and fine-tuned for 5 epochs per iteration. It stopped after iteration 10, identifying the model from this iteration as the last one meeting the threshold.
...
\textbf{Comparison (\texttt{compare\_results.py} Output):} The comparison script aggregated results from the base model and the outputs of both pruning methods. Table \ref{tab:results} summarizes the results from the tests.

% --- Results Table ---
% Using standard table environment
\begin{table*}[!t] % Use placement specifier like !t for top of page
    \renewcommand{\arraystretch}{1.3} % Increase row spacing slightly
    \caption{Comparison of Pruning Methods}
    \label{tab:results}
    \centering
    % Use \small or \footnotesize if table is too wide
    \begin{tabular}{@{}llrrrr@{}}
        \toprule
        Type    & Name                                          & Accuracy (\%) & Sparsity (\%) & Non-Zero Params   & GFLOPs \\ % Escaped %
        \midrule
        Base    & base\_model.pth                               & 85.49         & 0.00          & 4,440,778         & 0.16   \\
        \midrule
        iGreedy & pruned\_model\_iter\_0\_sparsity\_0.0.pth     & 85.49         & 0.00          & 4,440,778         & 0.16   \\
        iGreedy & pruned\_model\_iter\_1\_sparsity\_20.0.pth    & 85.37         & 16.14         & 3,724,057         & 0.16   \\
        iGreedy & pruned\_model\_iter\_2\_sparsity\_36.0.pth    & 85.49         & 27.05         & 3,239,634         & 0.16   \\
        iGreedy & pruned\_model\_iter\_3\_sparsity\_48.8.pth    & 85.75         & 28.28         & 3,185,379         & 0.16   \\
        iGreedy & pruned\_model\_iter\_4\_sparsity\_59.0.pth    & 85.79         & 35.53         & 2,863,200         & 0.16   \\
        iGreedy & pruned\_model\_iter\_5\_sparsity\_64.5.pth    & 85.74         & 33.68         & 2,945,309         & 0.16   \\
        iGreedy & pruned\_model\_iter\_6\_sparsity\_67.2.pth    & 85.66         & 33.90         & 2,935,507         & 0.16   \\
        iGreedy & pruned\_model\_iter\_7\_sparsity\_71.6.pth    & 85.84         & 30.74         & 3,075,815         & 0.16   \\
        iGreedy & pruned\_model\_iter\_8\_sparsity\_77.3.pth    & 85.35         & 29.45         & 3,133,297         & 0.16   \\
        iGreedy & pruned\_model\_iter\_9\_sparsity\_81.8.pth    & 84.88         & 29.20         & 3,144,454         & 0.16   \\
        \midrule
        L1Reg   & final\_l1\_pruned\_model\_1e-05.pth          & 87.28         & 58.33          & 1,850,772         & 0.16   \\
        L1Reg   & final\_l1\_pruned\_model\_1e-04.pth          & 85.39         & 68.27          & 1,409,645         & 0.16   \\
        \bottomrule
    \end{tabular}
\end{table*}
% --- End Results Table ---
...
\\
\textbf{Plot Analysis:}
The comparison script also generated plots (saved in the \texttt{results/} directory, see Figures \ref{fig:accuracy_vs_sparsity} and \ref{fig:accuracy_vs_gflops}).
\begin{itemize}
    \item \textit{Accuracy vs. Sparsity:} This plot shows the baseline model at (85.49\%, high accuracy). The iGreedy points trace a path of increasing sparsity and generally decreasing accuracy, stopping near the specified threshold (84\%). The L1 Regularization result appears as multiple points, achieving good sparsity for comparable accuracy.
    \item \textit{Accuracy vs. GFLOPs:} This plot shows accuracy decreasing while the estimated GFLOPs remain relatively constant. This visually confirms that unstructured weight pruning, as measured by standard architectural analysis (\texttt{thop}), does not significantly alter the theoretical computation count.
\end{itemize}

% Using standard figure environment
\begin{figure*}[!t] % Use placement specifier like !t for top of page
    \centering
    \includegraphics[width=\columnwidth]{results/accuracy_vs_sparsity.png} % Use \columnwidth for single column width
...
\begin{itemize}
     \item \textbf{Geetika Khanna:} Initial problem formulation based on mid-semester report, literature review on pruning techniques, implementation of the base \texttt{SimpleCNN} model and initial training setup. Formulation 1 of objective function and Implementation and refinement of the iGreedy threshold pruning algorithm, running pruning experiments.
     \item \textbf{Archit Harsh:}  Formulation 2 of the objective function and implementation of the L1 regularization pruning method, development of the results comparison script (\texttt{scripts/compare\_results.py}), running pruning experiments.
     \item \textbf{Himal Sharma:} Implementation of utility functions including device handling, FLOPs calculation, debugging device compatibility issues (MPS float32/device errors) and SSL errors, integration of FLOPs metric, final report writing, analysis, and documentation (README), running pruning experiments.
\end{itemize}
